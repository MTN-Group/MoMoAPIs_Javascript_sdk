...
import { RESET_FORM, UPDATE_PRODUCT } from 'store/actions';
...
const Dashboard = () => {
    ...
    const sucessCallBack = (res, header, status) => {
        if (
            ...
            setPollTime(1);
            let requestCatagory =
                window.momo.auth[customization.pageData.requestCategory];
        ...
        status ? setResponseStatus(parseInt(status))
        : setResponseStatus(parseInt(res.status))
        setLoading(false);
        res.data ? setResponseJson(JSON.parse(JSON.stringify(res.data)))
        :setResponseJson(JSON.parse(JSON.stringify(res)));
        res.headers ? setResponseHeader(JSON.parse(JSON.stringify(res.headers)))
        :setResponseHeader(header);
        // handle test
    ...
    const sendRequest = () => {
        ...
        if(customization.pageData.requestCategory) {
            if(customization.pageData.requestCategory=='initUser')
            {
                if (
                    window.momo &&
                    window.momo &&
                    window.momo[customization.pageData.requestCategory]
                ) {
                    chooseAuthType('auth');
                    window.momo.initUser[customization.pageData.requestType]({
                        ...requestJson,
            ...
            // }
                ...
                alert(`You are unauthorized. Please do login ${customization.pageData.requestCategory} to continue`);
            }
        }
    };
