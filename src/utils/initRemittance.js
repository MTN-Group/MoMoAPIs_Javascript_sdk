...
import { moduleWrapperWithRemittances } from "../modules";
...
function tokenGenerator({
  ...
  if (sessionStorage.getItem("remittanceSdkToken")) {
    const tokenData = JSON.parse(sessionStorage.getItem("remittanceSdkToken"));
    ...
    if (
      ...
      moduleWrapperWithRemittances({ SUBSCRIPTION_KEY, accessToken, callbackUrl });
      onSuccess &&
        onSuccess("You can access remittance functions using : window.momo.auth");
    } else {
      getToken({
        username,
        pass,
    ...
    generateToken({
        ...
        onSuccess: (data = null) => {
            if (data) {
                ...
                onSuccessCallback &&
                    onSuccessCallback(
                        'You can access remittance functions using : window.momo.auth'
                    );
            } else {
                onFailure &&
                    onFailure(
...
const setExpireTokenInStorage = ({ access_token = '', expires_in = 3600 }) => {
    ...
    sessionStorage.setItem('remittanceSdkToken', JSON.stringify(tokenSaveData));
};
