...
function tokenGenerator({
  ...
  if (sessionStorage.getItem("remittancesSdkToken")) {
    const tokenData = JSON.parse(sessionStorage.getItem("remittancesSdkToken"));
...
function getToken({
    ...
    generateToken({
        ...
        pass,
        SUBSCRIPTION_KEY,
        url: process.env.MOMO_REMITTANCE_TOKEN_URL,
        onSuccess: (data = null) => {
            if (data) {
...
const setExpireTokenInStorage = ({ access_token = '', expires_in = 3600 }) => {
    ...
    sessionStorage.setItem('remittancesSdkToken', JSON.stringify(tokenSaveData));
};
