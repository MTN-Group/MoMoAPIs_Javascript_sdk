...
import { moduleWrapperWithCollection } from "../modules";
...
function tokenGenerator({
  ...
  if (sessionStorage.getItem("collectionSdkToken")) {
    const tokenData = JSON.parse(sessionStorage.getItem("collectionSdkToken"));
    ...
    if (
      ...
      moduleWrapperWithCollection({ SUBSCRIPTION_KEY, accessToken, callbackUrl });
      onSuccess &&
        onSuccess("You can access collection functions using : window.gsma.auth");
    } else {
      getToken({
        username,
        pass,
...
function getToken({
    ...
    generateToken({
        ...
        pass,
        SUBSCRIPTION_KEY,
        url:process.env.MOMO_COLLECTION_TOKEN_URL,
        onSuccess: (data = null) => {
            if (data) {
                ...
                onSuccessCallback &&
                    onSuccessCallback(
                        'You can access collection functions using : window.gsma.auth'
                    );
            } else {
                onFailure &&
                    onFailure(
...
const setExpireTokenInStorage = ({ access_token = '', expires_in = 3600 }) => {
    ...
    sessionStorage.setItem('collectionSdkToken', JSON.stringify(tokenSaveData));
};
